...
class MembersController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Members']))
		{
			...
			$password=$_POST['Members']['password'];
			$model->password = md5($password);

			$model->foto_diri=CUploadedFile::getInstance($model,'foto_diri');

			// die($uploadedFile);
			
			if($model->save())
			{
				$model->foto_diri->saveAs(Yii::getPathOfAlias('webroot').'/images/'.$model->foto_diri);
				// $model->foto_diri->saveAs(Yii::getPathOfAlias('webroot').'/images/'.$fileName);  // image will uplode to rootDirectory/banner/
